...
- `src/api.js` — all API calls; uses `VITE_API_URL` env var; password sent via `x-character-password` header
  ...
  - `getNpcLibrary(dmPassword)` — GET /npc-library; DM auth; returns `{ templates: [{ id, name, abilities, hpMax, portraitUrl, updatedAt }] }`
...
- 22 Lambda handlers in `backend/src/handlers/`: `list`, `get`, `create`, `update`, `delete`, `verify`, `portrait`, `session`, `dmParty`, `initiative`, `dmNotes`, `getMapLibrary`, `mapPresign`, `postMap`, `putMapActive`, `patchMap`, `deleteMap`, `getPartyStatus`, `getInitiativePublic`, `getNpcLibrary`, `putNpcLibrary`, `npcPortraitPresign`
...
- `backend/src/lib/specialItems.js` — sentinel slug constants (`INITIATIVE_SLUG`, `NPC_COMBAT_SLUG`, `ROLL_HISTORY_SLUG`, `MAP_LIBRARY_SLUG`, `PARTY_ROSTER_SLUG`, `NPC_LIBRARY_SLUG`) and `filterPublicCharacterItems()` — single source of truth for sentinel filtering
- `backend/src/lib/specialRecords.js` — helpers for reading/writing sentinel items: `getMapLibraryState()`, `saveMapLibraryState()`, `normalizeMapLibraryRecord()`, `getNpcLibraryState()`, `saveNpcLibraryState()`, `normalizeNpcLibraryRecord()` — NOTE: `normalizeNpcLibraryRecord` passes through `hpMax` and `portraitUrl` (critical — do not strip them)
