...
- `src/api.js` — all API calls; uses `VITE_API_URL` env var; password sent via `x-character-password` header
  ...
  - `getSessionState({ dmPassword, slug })` — GET /session-state; **Story 35**, the single consolidated polling endpoint. `DmDashboardPage.jsx` and `CharacterModePage.jsx` each poll this via one `useAdaptivePolling` instance instead of 5 and 4 separate endpoint calls respectively. DM variant (pass `dmPassword`): `{ party, initiative, npcCombat, rollHistory, mapLibrary, counterWheels, serverTime }`. Public variant (omit `dmPassword`, optionally pass `slug`): `{ partyStatus, initiativePublic, mapLibrary, rollHistory, serverTime, character? }`. The old per-resource endpoints (`getDmParty`, `getInitiative`, `getNpcCombat`, `getRollHistory`, `getMapLibrary`, `getCharacter`, `getPartyStatus`, `getInitiativePublic`) remain in `api.js` and are still used by non-polling one-shot callers (`CharacterPage.jsx`, `MapLibraryPage.jsx`, `MapViewerPage.jsx`, the DM dashboard's mount-time auth check) — deprecated for polling use only, not removed.
...
- 30 Lambda handlers in `backend/src/handlers/`: `list`, `get`, `create`, `update`, `delete`, `verify`, `portrait`, `session`, `dmParty`, `initiative`, `dmNotes`, `getMapLibrary`, `mapPresign`, `postMap`, `putMapActive`, `patchMap`, `patchMapTokens`, `patchMapCalibration`, `moveMapToken`, `deleteMap`, `getPartyStatus`, `getInitiativePublic`, `getNpcCombat`, `putNpcCombat`, `getCounterWheels`, `putCounterWheels`, `getNpcLibrary`, `putNpcLibrary`, `npcPortraitPresign`, `getSessionState`
  ...
  - `getSessionState.js` — GET /session-state; **Story 35 consolidated polling endpoint**, replaces the per-resource polling above for `DmDashboardPage.jsx` and `CharacterModePage.jsx`. DM variant (valid DM password in `x-character-password`): `{ party, initiative, npcCombat, rollHistory, mapLibrary, counterWheels, serverTime }`. Public variant (no/invalid DM password): `{ partyStatus, initiativePublic, mapLibrary, rollHistory, serverTime }`, plus `character` when `?slug=<slug>` is supplied (same shape/stripping rules as unauthenticated `GET /characters/{slug}`). Reads: one `BatchGetItem` for the sentinel records (`initiative`, `npc-combat`, `roll-history`, `map-library`, `party-roster`, `counter-wheels` — `npc-library` intentionally excluded, not polled), one `BatchGetItem` for party-member characters keyed off the roster (plus the `?slug` target for the public variant) — two DynamoDB round trips max. Uses shared projection helpers (see below) so its output matches the old per-resource endpoints field-for-field. The old endpoints are NOT removed — they remain live for non-polling one-shot callers (e.g. `MapLibraryPage`).
