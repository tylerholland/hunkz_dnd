...
- 23 Lambda handlers in `backend/src/handlers/`: `list`, `get`, `create`, `update`, `delete`, `verify`, `portrait`, `session`, `dmParty`, `initiative`, `dmNotes`, `getMapLibrary`, `mapPresign`, `postMap`, `putMapActive`, `patchMap`, `deleteMap`, `getPartyStatus`, `getInitiativePublic`, `getNpcCombat`, `putNpcCombat`, `getCounterWheels`, `putCounterWheels`
...
- `backend/src/lib/specialRecords.js` — helpers for reading/writing sentinel items: `getMapLibraryState()`, `saveMapLibraryState()`, `normalizeMapLibraryRecord()`, `getNpcLibraryState()`, `saveNpcLibraryState()`, `normalizeNpcLibraryRecord()`, `getCounterWheelsState()`, `saveCounterWheelsState()`, `normalizeCounterWheelsRecord()`
