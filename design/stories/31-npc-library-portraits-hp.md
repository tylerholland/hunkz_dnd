...
1. **Does this story require Story 29b to have shipped first, or can they be developed in parallel?** The `portraitUrl` field on NPC combat objects is introduced by Story 29b. If Story 31 ships independently, it needs to introduce that field itself. Recommendation: sequence Story 29b first since it's a smaller story; then Story 31 builds on the established field. The architect should confirm. — **Resolved: Sequence 29b first.** 29b is the story that establishes the token render contract (`portraitUrl` on the `npc-combat` object + initials fallback). 31 must not re-implement the token portrait pipeline. If schedule pressure forces 31 to start before 29b ships, 31 may introduce `portraitUrl` on the spawned NPC object itself (the `putNpcCombat` pass-through already persists it — no normalizer change), but the token *render* + scale + fallback work stays in 29b's scope. Do not duplicate it.

2. **Portrait presign endpoint: extend the existing `/maps/presign` or add a new `/npc-library/portraits/presign`?** The map presign uses the same S3 bucket. Reusing it is operationally simpler but muddies the semantic boundary. A dedicated endpoint is cleaner but adds a Lambda. The architect should decide. — **Resolved: New dedicated `POST /npc-library/portraits/presign` handler (ADR-018).** `mapPresign.js` is map-shaped: it hard-codes the `maps/` key prefix, a 50MB cap, and accepts `application/pdf`. Reusing it would put NPC portraits under `maps/` and inherit the wrong limits. Clone it into `npcPortraitPresign.js` with an `npc-portraits/` prefix, image-only content types, and a 5MB cap. The extra Lambda is acceptable per ADR-004 at current handler count.

3. **Library editor location on the DM dashboard**: does the library management UI live in a dedicated section on the DM dashboard (below NPC combat, always accessible), or is it a modal/drawer opened from the Add Enemy form? Given that the DM may want to build the library between sessions (not during combat), some persistent access point outside the combat form seems desirable. The UX designer should decide the entry point. — **Resolved by UX design: modal advance editor, opened from a `⚙ Enemies Gallery` top-bar button** (and from the picker link). Not a persistent dashboard section. Architect concurs — a modal avoids adding always-mounted weight to the ~1100-line dashboard page.

4. **How does the DM see which live NPC cards came from a library entry?** If the card tracks its library source (a `librarySourceId` field), the save-back flow can offer "Update existing '[Name]'" contextually. If not, the match is done by name at save time (same as Story 24). The name-match approach is simpler but fragile if the DM renames the NPC mid-session. The architect should decide whether to track provenance. — **Resolved: Track `librarySourceId` on spawned NPC objects.** Stamp the source template `id` onto each spawned NPC. This is free — `normalizeNpcCombatRecord` spreads `...npc`, so the field persists with no backend change. The save-back flow keys the "Update existing" affordance off `librarySourceId` first, falling back to name-match when it's absent (Path 1 cards built from scratch in combat). This survives mid-session renames, which is the exact fragility the consultant flagged for Path 3.

5. **Portrait image size limits and validation**: should the presign flow enforce maximum file size or mime type at the Lambda level? The map upload flow has a size warning UI but no server-side hard cap. Consistent enforcement would be ideal. The architect should specify. — **Resolved: Yes, enforce at the Lambda.** The new presign handler requires a finite `size`, rejects `size > 5 * 1024 * 1024` with `badRequest`, and rejects any `contentType` not starting with `image/` (no PDF for portraits). Mirror `mapPresign.js`'s validation block exactly, swapping the cap and content-type set. Note this is a presign-time *declared*-size check, not a true upload cap (the browser declares the size); that is the same guarantee `mapPresign` gives and is sufficient at this trust scale.

6. **Auto-numbering when spawning count > 1**: when the DM spawns 3 Goblins from the library, should names be auto-suffixed ("Goblin 1", "Goblin 2", "Goblin 3") or left identical and differentiated only by their NPC card order? Identical names work fine for the card list (cards are ordered rows) but create confusion when the same name appears multiple times in the initiative tracker. Recommendation: auto-suffix is the right default and should be opt-out rather than opt-in. The UX designer should decide how prominently to surface this behavior at spawn time. — **Resolved: Auto-suffix, opt-out via `☑ Number them` (default checked), Count = 1 never numbered.** Per UX design. Suffix logic lives entirely client-side in the spawn handler — append ` ${i+1}` to each spawned NPC's `name`. No backend or data-model change.

7. **Token visual differentiation for identical portraits on the map**: D&D Beyond Maps solves this with a colored border ring per token; Roll20 uses token markers. This app's battle map token layer (Story 29) uses `portraitUrl` directly — there is currently no mechanism to distinguish three Goblin tokens that share the same portrait image. For v1 the name suffix on the NPC card (open question 6) is sufficient since tokens are not interactively selectable on the map today. If Story 29 evolves toward clickable tokens with identity, this will need revisiting. The architect should note whether the token layer shape already has a slot for a color indicator or label overlay. — **Resolved: No new token data. Derive a number badge from the name suffix at render time.** The token data shape (Story 29 / ADR-017 `tokens[]`) has no color/label slot, and we are not adding one. The UX design's 16px badge parses the trailing integer from the NPC `name` (`/\s(\d+)$/`) — this is purely a render concern and depends only on open question 6's naming, not on any stored token field. The badge render is in-scope for whichever story owns the token layer (29b); 31 only guarantees distinct names.

8. **Named villain vs. generic creature type in the library**: a recurring villain (e.g., "Malachar the Undying") is a single unique entity — the library entry IS the creature, and the DM will likely only ever spawn one instance. A generic creature type (e.g., "Goblin") is a template that might be spawned in groups of 5 every session. These two use patterns are meaningfully different, but the data model does not distinguish them. Should the library expose a "type" toggle (Unique / Generic) that changes the default spawn UX — e.g., Unique entries default to count 1 and are not auto-numbered, while Generic entries default to the numbered multi-spawn path? This is optional for v1 but worth flagging before design begins. — **Resolved: Defer to v2. Out of scope for this story.** The Count stepper defaults to blank and the DM types the count every spawn, so a Unique/Generic toggle saves at most one interaction and adds a field + UI mode the data model must carry forever. The `☑ Number them` opt-out already covers the "spawned one villain, don't number it" case. Do not add a `type` field to the library shape in v1.

---

## Architect Notes

**Applies**: ADR-003 (sentinel item pattern), ADR-004 (one Lambda per op), ADR-005/ADR-007 (DM auth via header), ADR-008 (S3 presigned-URL upload), ADR-018 (new — NPC portrait presign + library shape expansion)

**Tech approach**: This is a backwards-compatible *expansion* of infrastructure that already exists, not net-new. The `npc-library` sentinel, `GET`/`PUT /npc-library` handlers, `getNpcLibraryState`/`saveNpcLibraryState`, and `NPC_LIBRARY_SLUG` were all shipped by Story 24. The two backend changes are: **(1)** widen `normalizeNpcLibraryRecord` in `backend/src/lib/specialRecords.js` to pass through `hpMax` and `portraitUrl` — it currently filters each template down to `{ id, name, abilities, updatedAt }` and **will silently drop the two new fields on every read until edited**. This is the single most important backend touch and the easiest to miss. Add `hpMax: Number.isFinite(t.hpMax) ? t.hpMax : null` and `portraitUrl: typeof t.portraitUrl === "string" ? t.portraitUrl : null`. **(2)** Add `npcPortraitPresign.js` (ADR-018) plus its route in `template.yaml`, cloned from `mapPresign.js` with an `npc-portraits/` key prefix, image-only content types, and a 5MB declared-size cap. No change is needed to `putNpcCombat`/`normalizeNpcCombatRecord` — both already pass `portraitUrl` and `librarySourceId` through via `...npc` spread. `PUT /npc-library` remains a full-array replacement (ADR-004 contract, same as `putNpcLibrary`). Frontend follows ADR-001/ADR-014 (CSS custom properties, no runtime style injection) and ADR-011 (no polling — the library is fetched on dashboard mount and after each save, never polled, exactly as Story 24 established). Auto-numbering and the token number badge are pure client-side render logic (parse trailing integer from `name`) — zero data-model cost.

**Scope boundary**:
- *In*: expand `npc-library` template shape with `hpMax?` + `portraitUrl?`; widen `normalizeNpcLibraryRecord`; new `POST /npc-library/portraits/presign`; in-combat library picker in Add Enemy form; `⋯` save flow with 3-choice name-conflict; advance editor modal (list rail + editor panel, portrait replace/remove, duplicate, two-step delete); MRU sort; search >20 entries; spawn pre-fill (name/HP/abilities/portrait) with HP override and count; auto-suffix naming (opt-out); `librarySourceId` stamped on spawned NPCs.
- *Out*: full stat blocks; player visibility; external import; tags/folders; per-entry roll buttons; undo for delete; auto-propagation of mid-session edits to the library; Unique/Generic type toggle (OQ8, deferred to v2); the token number badge *render* and token portrait pipeline (owned by Story 29b — 31 only guarantees distinct names + supplies `portraitUrl`).

**Performance notes**: The advance editor is a modal, not an always-mounted dashboard section — keeps it off the ~1100-line `DmDashboardPrototypePage` render path until opened. Library is fetched on mount + after writes, never polled (ADR-011), so it adds no recurring read traffic. Search filter only mounts past 20 entries; at expected campaign scale (50–80 entries) a client-side `.filter()` over the in-memory array is fine — no backend search needed.

**Cost notes**: One new Lambda (`npcPortraitPresign`) — invoked only on portrait upload (rare, DM-only), negligible cost and cold starts are acceptable for an interactive editor action (ADR-004). NPC portraits reuse the existing `hunkz-dnd-portraits` bucket under an `npc-portraits/` prefix — no new bucket, no new storage class. At a handful of users and a few dozen creature portraits, S3 storage + request cost is effectively zero. The library is a single DynamoDB item read/written as a whole (ADR-003), so no per-entry read amplification.

**Dependencies**: Story 29b must ship first — it establishes `portraitUrl` on the `npc-combat` object and the token render/fallback/scale contract that this story's spawned-NPC portraits flow into (see OQ1). 31 supplies the library-side `portraitUrl` and stamps it onto spawned NPCs; it does not own the token render. No other feature or data field is a prerequisite.

**Risks / decisions needed**: The most likely implementation bug is forgetting to widen `normalizeNpcLibraryRecord` — the `PUT` will accept `hpMax`/`portraitUrl`, they persist to DynamoDB, but the next `GET` strips them and the UI shows blank HP/no portrait, looking like a write failure when it's a read-projection bug. Verify the normalizer change with a round-trip first. Second risk: the 255-char ability cap the consultant flagged (from Story 23) must NOT be enforced on the library side — DM reference lines ("Multiattack: 2 claws +5...") routinely exceed it; confirm no shared validation imposes it on library abilities. No open decisions remain for the user — all eight open questions are resolved above.
