...
Three related visual-effects stories on the battle-mode token layer, built together since they all touch the same `TokenChip` component (`BattleModeController.jsx`) and share a new module, `src/features/dmDashboard/battleMode/tokenEffects.js` (intensity-tier/freshness-gate/AoE-stagger/size-band/veil logic, condition→family/rank/glyph table, the token-exit ghost registry hook). A new `ConditionGlyphSprite.jsx` renders the 14-condition SVG glyph sprite once per map surface.
