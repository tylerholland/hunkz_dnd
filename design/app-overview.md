...
5. **Four-tab strip: Inventory | Persona | Combat | Map** — full-width icon+label tab strip (64px tall, SVG icon above label text, IM Fell English 13px uppercase tracked); active tab shows `pal.accentDim` background + `pal.accent` border + `pal.accentBright` text/icons; tab state stored in `sessionStorage` as `dnd_tab_${slug}`, default `"combat"`. The Map tab is visually dimmed (opacity 0.4, `cursor: not-allowed`) when no active map is set.
   ...
   - **Combat tab** (`combatTab === "combat"`): live-session resource surface; no password required to update any value
     ...
     - **Dice Roller** (`DiceRoller.jsx`): collapsible section at the bottom of the Combat tab (collapsed by default; state stored in `sessionStorage` as `dnd_dice_open_${slug}`). Contains:
       ...
       - **Ability check circles**: six tappable stat circles (STR/DEX/CON/WIS/INT/CHA); each rolls **2d6 + computed ability modifier** (house rule); advantage/disadvantage rolls a third d6 and keeps the best/worst two — all three dice shown in the breakdown row with the dropped die struck-through and dimmed; no crit/fumble on 2d6
     ...
     - **Expression text input**: accepts freeform expressions like `2d6+1d4+3`; overrides picker on roll when non-empty; Enter key triggers roll
      - **Result display**: single-die or multi-group layout; CSS-only spin animation (1050ms); staggered group reveal for combos; gold crit / red fumble number glow + label; d20 advantage/disadvantage shows both dice with discarded value struck through; 2d6 ability check advantage/disadvantage shows all three d6 results with the dropped die struck-through and dimmed at 38% opacity
      ...
      - **(Story 57)** A roll fired from the Attack Declaration Bar (Map sub-tab) carries the same broadcast, plus two optional fields, `target: { type: "npc", sourceId, name }` and `attack: { kind, id, name }` — never baked into the roll's own `label`. `RollHistoryRow` renders it as `→ ◎ Goblin 2` appended to the action-label line, on both the player's own local history and the DM's shared feed (same component). A roll fired from the panel's own weapon/ability buttons carries neither field and is unaffected.
...
| `exhaustionLevel` | number | 0–6; default 0; public/player-visible field (Story 53, map condition badge) |
| `lastDamagedAt` / `lastDamageAmount` | string (ISO) / number | Story 52 — damage-flash sync stamp; set by `session.js` whenever a `PATCH .../session` write strictly decreases `hpCurrent`; not secret data, in both DM and player-visible projections |
| `lastDamageFrom` | `{ type: "character"\|"npc", sourceId: string } \| null` | Story 55 — attacker ref, stamped in the SAME write as `lastDamagedAt`/`lastDamageAmount` (never a separate write), resolved server-side from `initiative.entries[activeTurnIndex]`; DM sees it unstripped, the public projection strips it when the attacker is invisible or linked to a hidden initiative entry (ADR-023) |
...
- Polls (Story 35): a single `getSessionState({ slug })` call via one `useAdaptivePolling` instance, fanned out to `data` (character), `mapLibrary`, `partyStatus`, and `initiativeData` state — replaces four separate polled endpoint calls with one request per tick. **Story 36**: while `useSessionSocket`'s WebSocket is connected, the visible/focused interval relaxes from `1s` to `30s`; a dropped/absent socket falls back to plain ADR-011 cadence
...
- `GET /session-state` (`getSessionState.js`, Story 35, extended Story 35b, Story 36b) — the consolidated polling endpoint. `DmDashboardPage.jsx`, `CharacterModePage.jsx`, `CharacterPage.jsx` (the classic full sheet at `/characters/:slug`), and `MapViewerPage.jsx` (`/map-view`) all poll this instead of the endpoints above — every polled page in the app now issues exactly one HTTP request per poll tick. DM variant (valid DM password): `{ party, initiative, npcCombat, rollHistory, mapLibrary, counterWheels, serverTime, buildVersion }`. Public variant (no/invalid DM password): `{ partyStatus, initiativePublic, mapLibrary, rollHistory, serverTime, buildVersion }`, plus `character` when `?slug=` is supplied (same stripping rules as unauthenticated `GET /characters/{slug}`, including `playerNotes` visibility keyed off the supplied `x-character-password`). `buildVersion` (Story 36b) is `null` until the first 36b deploy writes the `app-meta` sentinel — see "Live sync — stale-client auto-refresh" below. The endpoints above remain live and unchanged for true one-shot/mount-fetch callers (e.g. `MapLibraryPage`, NPC library, counter wheels) — deprecated for polling use, not removed.

**Live sync — WebSocket nudge channel (Story 36)**:
- A tiny push channel sits alongside (not instead of) the polling above. `DmDashboardPage.jsx` and `CharacterModePage.jsx` (and via a `wsConnected` prop, `CharacterSheetSessionMode.jsx`'s session-mode top bar) each hold a `useSessionSocket(onChanged)` connection. Every session-write handler on the backend fires a fire-and-forget `notifySessionChanged()` after a successful write; connected clients receive a bare `{"type":"changed"}` push and immediately refetch `GET /session-state` — sub-second sync instead of waiting for the next poll tick.
- No state travels over the socket, only the nudge — see ADR-019 ("nudge, not payload").
- While a client's socket is connected, its adaptive poll interval relaxes from 1s to the same 30s "safety net" already used for backgrounded tabs (ADR-011/Story 35); if the socket drops (network blip, tab hidden, etc.) the client falls back to the normal 1s/30s ADR-011 cadence with no user-visible error.
- A small pulsing-dot "Live"/"Polling" indicator appears in each page's top bar — cosmetic only, nothing else depends on connection state.
- Requires a deployed `VITE_WS_URL`; if unset, the whole feature is a no-op and every client behaves exactly as it did before Story 36.

**Live sync — stale-client auto-refresh & deploy reload broadcast (Story 36b)**:
- Deploys are now self-propagating: `deploy.sh` embeds its git short hash as `VITE_BUILD_VERSION` in the frontend build, writes the same hash to a new `slug: "app-meta"` sentinel (`{ buildVersion, deployedAt }`) via a direct `aws dynamodb put-item` call after the S3 sync completes, then invokes a new no-HTTP-route Lambda (`broadcastReload.js`) via `aws lambda invoke` to push `{"type":"reload"}` over the Story 36 WebSocket. Order is sync → version write → broadcast, so a reloading client always finds the new bundle already in place.
- `src/lib/staleClient.js` is the single client-side owner of "should this tab reload, and when is it safe": `reportServerBuildVersion(serverBuildVersion)` (called by `DmDashboardPage.jsx`, `CharacterModePage.jsx`, `CharacterPage.jsx`, and `MapViewerPage.jsx` on every `GET /session-state` response) schedules a reload on a version mismatch; `handleReloadBroadcast()` (called by `useSessionSocket.js` on a `{"type":"reload"}` message) does the same, bypassing the version compare. Both funnel through one safe-reload path: deferred while a text input/textarea is focused or a modal overlay is open (retried every second until safe), and guarded by a `sessionStorage` loop guard (`dnd_reloaded_for_<version>`) capping reloads to once per target version per tab per 5 minutes, so a mid-sync mixed S3 bundle can't trigger a reload storm.
- Complete no-op in local dev (`VITE_BUILD_VERSION` unset) and backward compatible with pre-36b deploys (`buildVersion: null` when the `app-meta` sentinel hasn't been written yet) — no client ever reloads in either case.
- Out of scope (unchanged from the story): migrating tabs already running a pre-36b bundle, service workers/PWA cache control, and any user-facing "update available" UI — reloads are silent and automatic.
...
- `dnd_reloaded_for_<version>` (Story 36b) — loop-guard timestamp written by `src/lib/staleClient.js`; caps stale-client auto-reload to once per target version (or the fixed `"broadcast"` target for a WebSocket reload push) per tab per 5 minutes
...
- **Round counter** (Story 19): shown in the initiative header row alongside "Initiative Order" / "Clear ×". Shows `1` when entries exist, `–` (em dash) when empty. Auto-increments with a brief `accentBright` brighten animation each time "Next Turn" wraps from the last entry back to the first. Manual ± steppers for correction (no hold-to-repeat; no auto-increment animation on manual changes). "Clear ×" resets round to 1 in the same write. `round` lives on the `slug:"initiative"` sentinel item, written atomically with `entries`/`activeTurnIndex`. Story 52 — the same sentinel also carries `turnStartedAt` (ISO timestamp), re-stamped by `PUT /initiative` whenever `activeTurnIndex`/`round` actually changes; clients use it to derive when a token's damage wound-halo should clear (on the transition *into* that entity's next turn) without a separate server-side clear write.
...
Four related visual-effects stories on the battle-mode token layer, built together since they all touch the same `TokenChip` component (`BattleModeController.jsx`) and share a new module, `src/features/dmDashboard/battleMode/tokenEffects.js` (intensity-tier/freshness-gate/AoE-stagger/size-band/veil/tracer-geometry logic, condition→family/rank/glyph table, the token-exit ghost registry hook). A new `ConditionGlyphSprite.jsx` renders the 14-condition SVG glyph sprite once per map surface.

- **Story 52 — Token Damage Flash.** A token that just took damage (`hpCurrent` decreased) gets a brief impact burst (recoil + red wash + shockwave, intensity tiered Standard/Heavy) followed by a quiet wound halo that persists until that entity's next initiative turn starts (or 12s in a fixed out-of-combat window). Backend stamps `lastDamagedAt`/`lastDamageAmount` on the character record (in `session.js`, piggybacking on the existing pre-write `GetCommand`) and on the NPC object (in `putNpcCombat.js`, via one added read). `initiative.js` stamps `turnStartedAt` on the initiative sentinel whenever the active turn/round advances; clients derive wound-halo liveness from `turnStartedAt` vs. `lastDamagedAt` rather than a server-side clear. All age arithmetic uses `GET /session-state`'s `serverTime`, never `Date.now()`. A freshness gate (stamp ≤4s old and newer than last-rendered) prevents a tab-return or map switch from replaying old impacts; AoE hits landing in the same poll tick stagger 70ms apart (cap 6, then wash-only).
- **Story 53 — Persistent Condition Indicator.** Up to 3 badges in a left-edge column inside each token (counter-scaled against `--token-size-mult` only, so badges stay a constant physical size regardless of per-token resize), sorted by a locked severity order (Incapacitating > Positional > Sense/will > Attrition), with `Exhaustion` folded in as a six-segment gauge badge when `exhaustionLevel ≥ 1`. A 4th+ condition collapses the bottom badge into a "more behind this" stack motif rather than a `+N` numeral. `Invisible` is excluded from the badge set on every viewer (it's Story 54's own treatment). Purely ambient — no resting animation, `pointer-events: none`, not interactive. The hover/long-press detail card gained a full condition-name list (superset of the badge column) on both PC and NPC tokens. NPC `conditions` and `exhaustionLevel` are now in the public (player-facing) payload — NPC conditions gated on the NPC's linked initiative entry not being hidden (the DM's existing secrecy lever, no new toggle); PC badges hide when `partyVisibilityEnabled` is off, except the player's own token, which always shows its own badges (the player's own full character record is merged into the token-resolution `party` array client-side for this reason).
- **Story 54 — Invisible Token Veil.** A character/NPC with the "Invisible" condition renders differently per viewer: a PC always renders VEILED (desaturated + dimmed portrait, dashed faction ring, slow shimmer) identically for every viewer including the DM — nobody loses track of their own party. An invisible NPC renders SECRET on the DM's map only (VEILED plus a diagonal hatch scrim and a `◇` marker in `#7c93a8`, the app's new "information only the DM can see" colour) and is **entirely absent** from every player's map — not hidden with CSS, not present in the DOM, not even shipped in the payload. The omission is server-side: a derived `invisible` boolean is computed once per token in `getSessionState.js` (both DM and public variants) and in the previously-unauthenticated, previously-verbatim `GET /maps` (`getMapLibrary.js`) — the latter is the leak a session-state-only fix would have missed. The flag is derived, never stored (`patchMapTokens.js` strips any client-supplied `invisible` field before persisting). A vanishing NPC on the player's map fades out over 500ms (diff-driven token-exit ghost registry in `tokenEffects.js`) rather than popping out of existence.
- **Story 55 — Attack Tracer.** When a damage-apply write resolves against a target (any path — dice roller "Apply to…", `DamageHealModal`, the ±1 HP stepper, and NPC HP edits alike), the attacker is inferred server-side from `initiative.entries[activeTurnIndex]` at the moment of that write and stamped as `lastDamageFrom: { type: "character"|"npc", sourceId }` in the SAME write as the existing damage stamp (`session.js`, `putNpcCombat.js`) — never client-inferred (the public initiative feed strips hidden entries, so a player's array doesn't index-align with the DM's). `"manual"` entries, no active combat, and self-damage all resolve to `null` (the last one auto-suppresses tracers for ongoing poison / start-of-turn damage). If both attacker and target resolve to placed tokens on the current map, a brief tracer plays: **Strike** (melee — token distance gap ≤ 1 combined-diameter unit) is a 6% attacker lunge toward the target and back plus three staggered velocity chevrons, nothing travels; **Bolt** (ranged weapon OR spell — the same visual for both, Channel is explicitly cut from v1) is a ~3px tapered streak with a gentle arc, 220–420ms by distance, tinted the attacker's palette accent for PCs / neutral steel for NPCs, rendered on a new `TracerLayer.jsx` below the token chips in z-order. Both terminate in a shared impact crescent on the target's rim. Choreography: crescent at t=0, Story 52's damage shockwave at t=+60ms (capped 480ms total) — this amends Story 52's previously-fixed 60ms into a supplied parameter, both maps consuming the same `tokenEffects.js` instance. Melee vs. ranged classification, all timing constants, and the concurrency caps (≤3 concurrent tracers, 250ms queue stagger beyond the cap) are named constants in `tokenEffects.js`, tunable after a live session. Zero new UI, zero new capture step — this is the story's core simplification, resolved explicitly during architecture rather than building a targeting flow.
- `lastDamageFrom` is stripped server-side (`getSessionState.js`'s public variant) whenever the referenced attacker is invisible or linked to a hidden initiative entry (reusing Story 54's invisible resolver and the hidden-entry set) — a tracer from an empty square would leak the attacker's position as effectively as rendering its token.
- The melee lunge lives on a new `.tk-lunge` DOM wrapper, OUTSIDE `.token-chip`'s existing counter-rotation (the single most likely implementation bug in this story — applied on the wrong layer, the lunge points backwards on a rotated map).
- Fail-open by design: if a token's subject can't be resolved, it renders un-flagged rather than being hidden from players with no `◇` on the DM's map (which would silently break the "DM's map is a strict superset of every player's map" invariant).
- Deferred/simplified in this pass (documented in the stories' implementation, not follow-up work yet): the Story 53 size-band resolver uses a plain 80ms debounce rather than the brief's debounce+2px-hysteresis combination; the exhaustion gauge and Phase-A/veil visuals are CSS approximations (conic-gradient fill, SVG dashed circle) rather than hand-tuned illustration; hatch-pitch counter-scaling and the exhaustion tier-escalation pulse were not implemented; Story 55's Channel (spell) visual and heal/support tracers are cut entirely (not stubbed) — Channel is tracked as a Story 57 follow-on once spells carry `role`/`toHit`/`damage` data.

### Attack Targeting Flow (session mode Map sub-tab, Story 57)

A player declares who they're attacking and with what, before rolling, from the battle-mode map itself — closing the gap Story 55's inference model worked around (it could guess *who* from initiative order but never *what*).

- **Gesture — additive, not an inversion of existing muscle memory.** On an NPC token: **tap** (<500ms, <8px movement) toggles the detail card open/closed; **hold** (≥500ms, <8px movement) declares that token as the target (or un-targets it if it's already the current target). Moving >8px cancels the hold and is read as an ordinary map pan — the two never collide because they're independent event families (`TokenChip`'s Pointer events vs. `MapViewer`'s pan on Mouse/Touch events), not a routing decision. Only armed when the active map is in Battle Mode **and** the character has at least one rollable weapon/attack-spell; otherwise every token behaves exactly as it always has, including the original **hover**-driven detail card (see below). PC tokens are untouched — targeting is NPC-only, own-token drag (Story 34) is PC-only, so the two gestures can never apply to the same token.
- **Fix (2026-08-11): tap-to-expand replaced hover-to-expand on armed NPC tokens.** The detail card originally opened on hover for every token, including NPC tokens while targeting was armed — but hover is incidental (the mouse can rest there for any reason) and its passive portrait-grow animation had no matching rule for the targeting reticle (`.tk-target-ring`), so the reticle stayed sized/positioned for the resting portrait for as long as the mouse hovered, visually detaching from the now-bigger portrait until mouseout. Fix: while targeting is armed (`canTarget`), hover no longer opens the card at all; a clean tap (below the hold threshold, no pan movement) toggles it instead, firing immediately on release with no extra delay. When targeting is disarmed (no battle mode, or no rollable attack), the token behaves exactly as before — hover still opens the card, tap remains a no-op. PC tokens and the DM's own map view are untouched; this only ever changes NPC tokens on the player's map.
- **Reticle**: a four-arc bracket ring outside the token's faction ring, tinted the viewing player's own palette accent (private intent — only the declaring player sees it; there is no shared/DM-visible reticle in v1). A charge-sweep arc fills during the 500ms hold (reusing the DM's existing long-press-ring `stroke-dashoffset` mechanism under a new class/threshold) so the gesture is legible while it's happening, not just after.
- **The Attack Declaration Bar** (`AttackDeclarationBar.jsx`, new) is fixed to the bottom of the session-mode viewport, present only while a declaration is live, and survives a sub-tab switch (it isn't inside any sub-tab panel — a caster can check Spell Slots mid-declaration without losing anything). Four states: **PICK** (target + a horizontally-scrolling picker of the same weapons-then-`✶`-spells list Story 56 already merges, spent spell slots shown greyed with a `SLOTS 0` tag and a shake-refusal on tap) → **ARMED** (chosen chip + `ATTACK`) → **RESULT (atk)** (total + `⚔ DAMAGE` / `↺ AGAIN`) → **RESULT (dmg)** (total + `↺ AGAIN`). The chosen chip's loaded expression (e.g. `1d20+7`) is tap-to-edit — a DM house-rule modifier not captured on the sheet doesn't block the roll — and a compact Advantage/Disadvantage strip appears for the to-hit step only, driving the same `advMode` the dice roller's own panel strip already shows (one shared state, not a second copy). Retargeting a different enemy while ARMED keeps the chosen weapon/spell (Extra Attack costs two taps per additional swing: `↺ AGAIN` then `⚔ DAMAGE`). Background taps clear a bare PICK declaration but do nothing once ARMED/RESULT — only the explicit `×` clears committed state.
- **Roll overlay**: landing a roll from this flow also plays a large centred number-reveal over the whole viewport (reusing the dice roller's cycling-number reveal at scale), in addition to — not instead of — the bar's own line-item result.
- **No new roll engine.** `DiceRoller.jsx`'s imperative handle gained `rollAttack` (alongside the existing `rollAbility`) and calls the exact same `executeRoll` pipeline every other roll in the app uses — same crit/fumble detection, same local history, same `postCharacterRoll` broadcast. Rolling from the Map sub-tab suppresses the roller panel's own force-open side effect so it doesn't pop open the Combat sub-tab underneath the player.
- **The roll feed gets no new row type.** `postCharacterRoll`'s payload gained two optional fields, `target: { type: "npc", sourceId, name }` and `attack: { kind, id, name }` — never baked into the existing `label` string. `RollHistoryRow` (shared by the player's own roller history and the DM dashboard's feed — same component, so the DM sees this for free) appends `→ ◎ Goblin 2` to the action-label line when `entry.target` is present. The feed always shows the *name captured at declaration time*, never a live lookup — an entry stays a correct historical statement even after the NPC is renamed, duplicated, or removed.
- **Target removed mid-declaration is not silent.** If the targeted token is absent from two consecutive polls (one degraded response can't flicker the bar away mid-turn), the name strikes through, turns red, gains a `— GONE` tag, holds 1400ms, then the bar exits. An NPC going Invisible (Story 54) takes this identical path for free, since it's already absent from the player's payload.
- **Out of scope for v1** (all deliberate, tracked as follow-ons, not oversights): no automatic hit/miss or AC comparison — damage application is still the DM's existing separate manual step; no multi-target/AoE selection; no targeting allies or heal-role spells; no range/line-of-sight validation; no DM-facing targeting UI or shared reticle; the classic full character sheet's Map tab (no token layer at all) is unaffected — session mode only.
...
Story 54 — every token in every `GET /session-state` and `GET /maps` response additionally carries a server-derived `invisible: boolean` (never persisted; `patchMapTokens.js` strips any client-supplied value). The public variants of both endpoints omit every `type: "npc"` token whose flag is `true` entirely — a true absence, not a client-side filter.
...
- ~~Spells are not individually rollable~~ **Closed by Story 57**: `role: "attack"` spells (and equipped weapons) are rollable from the Map sub-tab's new target-first Attack Declaration Bar via `DiceRoller`'s new `rollAttack` imperative-handle entry point. The roller panel's own weapon-roll-buttons row is still deliberately untouched — this is a second entry point into the same engine, not a change to the panel.
- **No DM-facing targeting UI, shared reticle, or "Apply to…" pre-selection from a declared target** (Story 57 OQ-5/OQ-6, explicitly deferred, flagged as candidate follow-on stories once `target.sourceId` is flowing through the roll feed)
- **Story 55's Channel (spell) tracer visual is still not built**: Story 57 lands the spell-shape data (`level`/`toHit`/`damage`) Channel needs, but capturing `attack.kind` at *roll* time doesn't close Story 55's tracer gap — the tracer still fires off a *damage-apply*, which carries no item reference. A correlation-window follow-on (a declared spell attack against the same `sourceId` within ~20s of a damage-apply to it) is the recommended path, not built here.
