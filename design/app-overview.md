...
- **Gesture — additive, not an inversion of existing muscle memory.** On an NPC token: **tap** (<500ms, <8px movement) is completely unchanged from today (the pre-existing hover detail-card behaviour); **hold** (≥500ms, <8px movement) declares that token as the target (or un-targets it if it's already the current target). Moving >8px cancels the hold and is read as an ordinary map pan — the two never collide because they're independent event families (`TokenChip`'s Pointer events vs. `MapViewer`'s pan on Mouse/Touch events), not a routing decision. Only armed when the active map is in Battle Mode **and** the character has at least one rollable weapon/attack-spell; otherwise every token behaves exactly as it always has. PC tokens are untouched — targeting is NPC-only, own-token drag (Story 34) is PC-only, so the two gestures can never apply to the same token.
